...
# Get the directory of 'ohos' and append 'data'
# This searches upwards from the current directory to find the 'ohos' folder
path_base = next((p for p in [Path.cwd()] + list(Path.cwd().parents) if p.name == 'ohos'), Path.cwd())
...
for r_ in range(R_):
    ...
    if save_:
        ...
        # Convert to string if necessary, though most libraries accept Path objects
        file = str(file_path)
