...
# hp.randint(label, low, high) generates an integer in [low, high)
space_obtree = {
    'max_depth': hp.randint('max_depth', 1, 11), # Results in 1-10
    's': hp.randint('s', 1, 11),                 # Results in 1-10
    'min_samples_leaf': hp.quniform('min_samples_leaf', 1, 20, 1),
    'min_samples_split': hp.quniform('min_samples_split', 2, 20, 1)
}

def objective_obtree_regression(space):
    X_train_in, X_train_out, y_train_in, y_train_out = space['data']
    
    # Extract and cast parameters to int
    # This prevents float/indexing errors in the algorithm
    m_depth = int(space['max_depth'])
    s_val = int(space['s'])
    m_leaf = int(space['min_samples_leaf'])
    m_split = int(space['min_samples_split'])
    b = int(space['b'])
    ...
    pro_tree.finalize(X_train_in, y_train_in)
    
    pro_tree.expanded_weights = pro_tree.used_weights
    
    # Ensure prediction handles the new depth correctly
    prediction = pro_tree.predict(X_train_out)
...
def objective_orf_regression(space):
    ...
    clf = ObliqueRandomForestRegressor(
                n_estimators=30, 
                max_depth = space['max_depth'],
...
def objective_porf_regression(space):
    ...
    clf = PatchObliqueRandomForestRegressor(
                n_estimators=30, 
                max_depth = space['max_depth'],
                max_features = min((X_train_in.shape[1])**2, 1000),
                min_samples_leaf = int(space['min_samples_leaf']),
